<?php
    ...
    if ($simpan AND $id != "" AND $kode != "" AND $nama != "" AND $harga != "") {	
        try {
            $query = $koneksi->prepare("UPDATE instan SET kode_barang=:kode, nama_barang=:nama, harga_barang=:harga WHERE id_barang=:id");
...
/** Proses memilih data */
    if ($id != "") {
        try {
            $query = $koneksi->prepare("SELECT * FROM instan WHERE id_barang=:id");
            $query->bindParam(":id", $id);
            $query->execute();
...
<div class="container">
	<div class="row">
		<div class="col-md-12">
			...
			<form action="?page=editInstan" method="post" enctype="multipart/form-data">
