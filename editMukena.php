<?php
    ...
    if ($simpan AND $id != "" AND $kode != "" AND $nama != "" AND $harga != "") {	
        try {
            $query = $koneksi->prepare("UPDATE mukena SET kode_barang=:kode, nama_barang=:nama, harga_barang=:harga WHERE id_barang=:id");
...
/** Proses memilih data */
    if ($id != "") {
        try {
            $query = $koneksi->prepare("SELECT * FROM mukena WHERE id_barang=:id");
            $query->bindParam(":id", $id);
            $query->execute();
...
<div class="container">
	<div class="row">
		<div class="col-md-12">
        <div><h3 class="text-center">Input Data Mukena</h3></div>
			<form action="?page=editMukena" method="post" enctype="multipart/form-data">
